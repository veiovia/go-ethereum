...
var (
	...
	ApplicationVeiovia = SigFormat{
		accounts.MimetypeVeiovia,
		0x05,
	}
	TextPlain = SigFormat{
...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	...
	case ApplicationVeiovia.Mime:
		// Clique is the Ethereum PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useEthereumV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationVeiovia.Mime)
		}
		veioviaData, err := hexutil.Decode(stringData)
		if err != nil {
			return nil, useEthereumV, err
		}
		header := &types.Header{}
		if err := rlp.DecodeBytes(veioviaData, header); err != nil {
			return nil, useEthereumV, err
		}
		// The incoming clique header is already truncated, sent to us with a extradata already shortened
		if len(header.Extra) < 65 {
		...
		messages := []*NameValueType{
			{
				Name:  "Veiovia header",
				Typ:   "veiovia",
				Value: fmt.Sprintf("veiovia header %d [0x%x]", header.Number, header.Hash()),
			},
		}
		// Clique uses V on the form 0 or 1
		useEthereumV = false
...
// in veiovia.go panics if this is the case, thus it's been reimplemented here to avoid the panic
...
// in veiovia.go panics if this is the case, thus it's been reimplemented here to avoid the panic
// and simply return an error instead
func veioviaHeaderHashAndRlp(header *types.Header) (hash, rlp []byte, err error) {
	if len(header.Extra) < 65 {
		err = fmt.Errorf("veiovia header extradata too short, %d < 65", len(header.Extra))
		return
	}
	rlp = veiovia.VeioviaRLP(header)
	hash = veiovia.SealHash(header).Bytes()
	return hash, rlp, err
}
