...
// Package veiovia implements the proof-of-authority consensus engine.
package veiovia
...
// Veiovia is the proof-of-authority consensus engine proposed to support the
// Ethereum testnet following the Ropsten attacks.
type Veiovia struct {
	config *params.VeioviaConfig // Consensus engine configuration parameters
...
// New creates a Veiovia proof-of-authority consensus engine with the initial
// signers set to the ones provided by the user.
func New(config *params.VeioviaConfig, db ethdb.Database) *Veiovia {
...
func (c *Veiovia) Author(header *types.Header) (common.Address, error) {
	return ecrecover(header, c.signatures)
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (c *Veiovia) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
...
func (c *Veiovia) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
func (c *Veiovia) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Veiovia) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Veiovia) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (c *Veiovia) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (c *Veiovia) VerifySeal(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// from.
func (c *Veiovia) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Veiovia) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// Seal implements consensus.AuthorizableEngine, attempting to create a sealed block using
// Authorize injects a private key into the consensus engine to mint new blocks
// with.
func (c *Veiovia) Authorize(signer common.Address, signFn consensus.SignerFn) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *Veiovia) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
...
func (c *Veiovia) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
func (c *Veiovia) SealHash(header *types.Header) common.Hash {
	return SealHash(header)
}

// Close implements consensus.Engine. It's a noop for veiovia as there are no background threads.
func (c *Veiovia) Close() error {
	return nil
}

// APIs implements consensus.Engine, returning the user facing RPC API to allow
// controlling the signer voting.
func (c *Veiovia) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "veiovia",
		Version:   "1.0",
		Service:   &API{chain: chain, veiovia: c},
		Public:    false,
	}}
}
...
// VeioviaRLP returns the rlp bytes which needs to be signed for the proof-of-authority
...
func VeioviaRLP(header *types.Header) []byte {
	b := new(bytes.Buffer)
...
func (c *Veiovia) nextDnaHash(s *Snapshot) (error, []string) {
	...
	log.Info("Calling databroker ", url, nil)
	r, err := http.Get(url)
