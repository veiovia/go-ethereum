...
// checkpoint creates a Veiovia checkpoint signer section from the provided list
...
// sign calculates a Veiovia digital signature for the given block and embeds it
...
// the account may or may not have cast a Veiovia vote.
...
// Tests that Veiovia signer voting is evaluated correctly for various simple and
// complex scenarios, as well as that a few special corner cases fail correctly.
func TestVeiovia(t *testing.T) {
	...
	for i, tt := range tests {
		...
		config.Veiovia = &params.VeioviaConfig{
			Period: 1,
			Epoch:  tt.epoch,
		}
		engine := New(config.Veiovia, db)
		...
		// Pass all the headers through veiovia and ensure tallying succeeds
