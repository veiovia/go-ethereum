...
func InspectDatabase(db ethdb.Database) error {
	...
	for it.Next() {
		...
		case bytes.HasPrefix(key, []byte("veiovia-")) && len(key) == 8+common.HashLength:
			veioviaSnaps.Add(size)
	...
	stats := [][]string{
		...
		{"Key-Value store", "Veiovia snapshots", veioviaSnaps.Size(), veioviaSnaps.Count()},
